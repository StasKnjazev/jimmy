use crate::{data::{InstallOptions, Partition, Kernel}, is_file};
use regex::Regex;
...
impl InstallOptions
{
    ...
    {
        if !self.check_valid_zoneinfo() {
            panic!("zoneinfo (region: '{}', city: '{}') not valid", self.region, self.city);
        }
        let lines: Vec<String> = vec![
    ...
    /// If the combination of region and timezone are valid, return true
    fn check_valid_zoneinfo(&self) -> bool
    {
        is_file(&format!("/usr/share/zoneinfo/{}/{}", self.region, self.city))
    }
